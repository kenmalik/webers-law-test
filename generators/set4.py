...
def _build_set1_three(save_figure):
    district_names = ["Toyota", "Honda", "Dodge", "KIA", "Lexus", "Subaru"]
    broken_hydrant_percent = np.array([50.1, 42.6, 35.9, 49.8, 41.3,  35.6], dtype=float)

    y_min, y_max = 30, 55
    y_ticks = np.arange(y_min, y_max + 1e-9, 1)
    ...
    ax.set_title("Set 4A — Average MPG of Car Brands")
    ax.tick_params(axis="x", rotation=20)
    p1 = save_figure(fig, "set4_no_aids.png")
    ...
    ax.set_title("Set 4B — Average MPG of Car Brands")
    ax.tick_params(axis="x", rotation=20)
    p2 = save_figure(fig, "set4_gridlines.png")
    ...
    ax.set_yticks(y_ticks)
    ax.grid(axis="y", alpha=0.0)
    ax.set_title("Set 4C — Average MPG of Car Brands")
    ax.tick_params(axis="x", rotation=20)
    p3 = save_figure(fig, "set4_fillbar.png")

    # Question + answer key (you can adjust wording - is ignored in .gitignore, record survey answers on your own)
    question = "Which percent difference is larger: Toyota vs Honda, or Dodge vs KIA?"
